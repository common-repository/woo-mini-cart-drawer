	...
	class API_Response {
		...
		public function set_response( $status, $message = '', $data = null ) {
			...
			if ( ! empty( $message ) && is_string( $message ) ) {
				if ( $status ) {
					\Appsbd_Lite\V2\Core\Kernel_Lite::add_info( $message );
				} else {
					\Appsbd_Lite\V2\Core\Kernel_Lite::add_error( $message );
				}
			}
			$this->msg  = \Appsbd_Lite\V2\Core\Kernel_Lite::get_msg_for_api();
			$this->data = $data;
